...
var aCategory = "";
//每次都会生成一个随机文章ID
var articleIdx = Math.floor(Math.random()*99999);
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for (var i = 0; i < data['categoryList'].length; i++) {
                    selectCategories.append($('<option class="categoriesOption" value="' + data['categoryList'][i]['cateName'] + '">' + data['categoryList'][i]['cateName'] + '</option>'));
                }
...
function publishSuccessPutIn() {
    $('#removeDiv').html('');
    var sec = $('<div id="all"></div>');
    var success = $('<div class="success"></div>');
    var successBox = $('<div class="success-box"></div>');
    var successArticleTitle = $('<div class="successArticleTitle"><span>' + articleTitle.val() + '</span></div>');
    ...
    var successTimeAndUser = $('<div class="success-time-user">' +
        '<p><i class="am-icon-calendar"></i>&nbsp;' + nowDate + '&nbsp;&nbsp;&nbsp;&nbsp;<i class="am-icon-user"></i>&nbsp;' + "mzy" + '</p>' +
        '</div>');
    var successBtn = $('<div class="successBtn">' +
        '<a href="/editor" class="reWriteBtn am-btn am-btn-danger am-round">写新文章</a>' +
        '<a href="/getArticleById?articleId=' + articleIdx + '" class="lookArticleBtn am-btn am-btn-danger am-round">查看文章</a>' +
        '</div>');
...
surePublishBtn.click(function () {
    ...
    if (articleTagsValue.length === 0 || articleTagsValue[tagNum - 1] === "") {
        ...
        var tie = articleTitle.val();
        var body = articleContent.val();
        $.ajax({
            method: "POST",
            url: "/publishArticle",
            data: JSON.stringify({
