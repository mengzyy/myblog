    ...
    success: function (data) {
        ...
        $.each(data['tagsList'], function (index, obj) {
            ...
            categoriesComment.append($('<div class="category">' +
                '<span><a class="categoryName" onclick="getArticlesByTagName(' + tagname + ')"' + '>' + obj['tagName'] + '</a></span>' +
                '</div>'));
        });
...
//获取特定标签文章和初始化第一页
function getArticlesByTagName(tagNamede) {
    tagName= tagNamede;
    //这里每次点击就刷新右边ajax
    //返回全部某类别数据,同时初始化插件
    $.ajax({
        type: 'GET',
        url: '/getArticlesByTagName',
        dataType: 'json',
        data: {"tagName": tagName},
        success: function (data) {
            tagsData = data['articleList'];
            //默认先画第一页
            nextpage(1);
        }
    })
...
function nextpage(datanum) {
    ...
    timeLine.append($('<div class="timeline-row">' +
        ...
        '</span>' +
        '<h1 class="title  am-animation-slide-top"># ' + tagName + '</h1>' +
        '</div>'));
